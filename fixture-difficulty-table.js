...
Promise.all([ipcRenderer.invoke("static-api"), ipcRenderer.invoke("fixture-api")]).then(([static, fixtures]) => {
  const teams = Object.fromEntries(static.teams.map((team) => [team.id,team.name]));
  const weeks = Object.fromEntries(static.events
      .filter((week) => !week.finished && !week.is_current)
      .map((week) => [week.id, week.name]));
    console.dir(Object.keys(weeks));
  ...
  const fixtureDifficultyData = static.teams.map(team => [
    ...
    weeks: static.events
      ...
      .map(week => {
        ...
        return [
          week.id,
          ...weekFixtures.map(wf => ({ teamId: wf.oppositionId, difficulty: wf.difficulty })),
        ];
      }),
  }));
