...
const FdrTeam = require("./team");

module.exports = {
  template: `
<div id="fdr-all-teams">
  <fdr-team
    v-for="team in teamData" :key="team.teamId"
  ...
  mounted() {
    ...
    Promise.all([ipcRenderer.invoke("static-api"), ipcRenderer.invoke("fixture-api")]).then(([static, fixtures]) => {
      ...
      vm.teamData = static.teams.map(team => ({
        teamId: team.id,
        color: color(team.id),
        weeks: static.events
    ...
    //     vm.$refs.charts.innerHtml = "";
    //     const svg = d3.select(vm.$refs.charts)
    //       .append("svg")
